...
const PreOrdersManagementPage = () => {
  ...
  const handleSavePreOrder = async (
    ...
    try {
      ...
      // Check if it's a new pre-order or not
      ...
      // Format the data before saving - explicitly remove properties we don't want to send
      const { customer, flight, items, total_amount, remaining_amount, advance_payment, preorder_id: _, ...preOrderData } = preOrder;

      // Create the formatted pre-order object
      const formattedPreOrder = {
        // For new pre-orders, preorder_id should be omitted so the database trigger generates it
        ...(isNewPreOrder ? {} : { preorder_id }),
        customer_id: preOrderData.customer_id,
        flight_id: preOrderData.flight_id || null, // Allow null for flight_id
        order_status: preOrderData.order_status,
        subtotal: calculatedSubtotal,
        delivery_charges: preOrderData.delivery_charges || 0,
        created_at: new Date(preOrderData.created_at || new Date()).toISOString()
      ...
      if (productItems && productItems.length > 0) {
        ...
        // Format the product items - now including advance_payment
        const formattedItems = productItems.map(item => ({
          ...
          price: item.price || 0,
          link: item.link || '',
          advance_payment: item.advance_payment || 0 // Add advance_payment to items
        }));
      ...
      // If it's a new pre-order (doesn't have a preorder_id) and advance payment > 0,
      // automatically create a payment record without affecting the remaining amount
      if (!preOrder.preorder_id && preOrder.advance_payment > 0) {
        try {
          console.log('Creating automatic payment for advance payment:', preOrder.advance_payment);
          ...
          const paymentData = {
            ...
            preorder_id: savedPreOrderId,
            amount: preOrder.advance_payment,
            payment_purpose: 'advance',
            bank_account: bankAccount, // Use the selected bank account
            tally: false,
            payment_date: new Date().toISOString().split('T')[0]
          };

          console.log('Automatically creating payment record:', paymentData);

          // Insert the payment directly without updating the preorder's remaining amount again
          ...
          if (paymentError) {
            console.error('Error creating automatic payment record:', paymentError);
            ...
            toast.success('Advance payment record created automatically');
          }
        } catch (paymentErr: any) {
          console.error('Error in automatic payment creation:', paymentErr);
  ...
  useEffect(() => {
    ...
    fetchAllItems();
  }, [preOrders, currentPage, searchQuery, statusFilter, flightFilter, fetchPreOrderItems, preOrderItems, loadingItems, getPaginatedData]);
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (row: PreOrderWithDetails) => {
        ...
        // If we don't have items yet and aren't loading, just show loading message
        // We'll use a useEffect to trigger the loading instead of calling it directly here
        
        // Instead of calling fetchPreOrderItems directly here (which triggers a state update during render)
        // we'll use useEffect in the component to load the items for visible preorders
        return <span className="text-gray-500 dark:text-gray-400">Loading items...</span>;
      },
      sortable: false,
